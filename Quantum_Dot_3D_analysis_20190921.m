function [P,Q,S,R,r]=Quantum_Dot_3D_analysis_20190921(a,b,t)
...
%Calibration Curve works since 20190921 data
...
end %reomve bad point with poor 
F(:,9)=[];F(:,8)=[];F(:,4)=[];F(:,3)=[];
[m,~]=size(F);
P=zeros(m,5);l=1;
while l<=m
    ...
    if x<0.83497 && x>-0.83390
        P(l,3)=F(l,3);P(l,4)=F(l,4);
        P(l,5)=725.88598+log10(1.66887/(x+0.83390)-1)/0.00315;
        l=l+1;
    else
        l=l+1;
    end 
...
P(:,2)=[]; %remove I0
%r is the position from each time point to the last time point
[m,~]=size(P);r=zeros(m,2);
r(:,1)=P(:,1);l=1;
while l<=m
    r(l,2)=sqrt((P(l,2)-P(m,2))^2+(P(l,3)-P(m,3))^2+(P(l,4)-P(m,4))^2);
    l=l+1;
end
%r is the position from each time point to the last time point
[m,~]=size(P);
